...
# Global cache for models (each carries its scaler) to avoid reloading
_MODEL_CACHE = {}

# api.binance.com refuses some regions (e.g. US datacenters, HTTP 451),
# so fall back to Binance's public market-data mirror
BINANCE_BASE_URLS = ["https://api.binance.com", "https://data-api.binance.vision"]


def binance_get(path, params, timeout=10):
    """GET a public Binance endpoint, trying each base URL until one succeeds."""
    last_error = None
    for base_url in BINANCE_BASE_URLS:
        try:
            ...
            print(f"[WARN] Binance request to {base_url}{path} failed: {e}")
            last_error = e
    raise last_error
...
def get_live_data(symbol="BTC", interval="1h", usd_to_inr=88.75):
    ...
    try:
        data = binance_get("/api/v3/klines", params, timeout=10)
...
def get_realtime_price(symbol="BTC", usd_to_inr=88.75):
    ...
    try:
        data = binance_get("/api/v3/ticker/price", params, timeout=5)
...
def load_model_and_scaler(symbol="BTC", interval="1h"):
    ...
    model_file = f"{symbol}_{'hourly' if interval=='1h' else 'daily'}_lstm.npz"
    ...
    try:
        # Exported weights + scaler (see Model_Training/export_numpy_weights.py)
